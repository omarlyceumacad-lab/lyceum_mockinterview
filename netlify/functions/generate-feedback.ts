...
// --- Gemini API Configuration (copied from original service) ---
...
export const handler: Handler = async (event) => {
  ...
  try {
    const { scores, details } = JSON.parse(event.body || '{}');

    if (!scores || !details) {
        return { statusCode: 400, body: JSON.stringify({ error: "Missing scores or details in request body." }) };
    }
    ...
    return {
      ...
      body: responseText, // The response from Gemini is already a JSON string
    };
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: error.message || "An error occurred while generating feedback." }),
    };
  }
};
